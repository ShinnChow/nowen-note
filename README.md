...
v1.4.11 聚焦 **笔记副本、搜索正确性、移动端 Markdown 编辑、协作图片体验与编辑器细节优化**，同时继续完善 AI、备份、共享附件和多端发布流程。

- 笔记支持创建**独立副本**，可快速复用现有内容，并与原笔记保持彼此独立。
- 搜索重新以索引内容作为检索来源；同时修复知识树搜索状态下新建文档后“看不到新文档”的问题。
- 统一笔记右键菜单，修复导出子菜单显示；重复标题辅助下光标保持可见，桌面端图片变换操作也不再出现重复菜单。
- 移动端 Markdown 恢复编辑 / 实时预览切换与视图控制，并扩大可用编辑区域，修复移动端编辑器操作面板异常。
- 修复移动端协作笔记图片全屏预览黑屏，并恢复公开 / 共享笔记本中的附件访问。
- 修复 AI 知识库统计和聊天内容布局，恢复自动全量备份的保留清理；iOS / TestFlight 构建版本改为跟随 Release Tag，并修正 MCP 启动器路径说明。

查看：[v1.4.11 Release](https://github.com/cropflre/nowen-note/releases/tag/v1.4.11) · [完整更新日志](./CHANGELOG.md)
...
- 新增**创建独立副本**：可基于现有笔记快速生成新的独立内容，后续修改不会影响原笔记。
- 恢复索引内容作为搜索来源，保证搜索结果与当前索引保持一致。
- 修复知识树搜索状态下创建新文档后不可见的问题，新建内容可以立即在当前视图中正确反馈。

### 编辑器与 Markdown

- 统一笔记右键菜单行为，并修复导出子菜单的显示问题。
- 修复重复标题辅助状态下光标不可见的问题，输入体验更稳定。
- 桌面端图片变换操作集中到正确入口，不再显示重复的变换菜单。
- 移动端 Markdown 恢复编辑 / 实时预览切换、视图控制和模式状态，并扩大实际可用的编辑区域。

### 移动端、协作与共享

- 修复移动端协作笔记图片全屏预览黑屏。
- 修复移动端编辑器操作面板和紧凑布局相关显示异常。
- 恢复公开 / 共享笔记本中的附件访问，避免共享内容里的图片或附件无法正常加载。

### AI、备份与发布

- 修复 AI 知识库统计与聊天内容布局问题。
- 恢复自动全量备份的保留清理逻辑，避免历史备份无限堆积。
- iOS / TestFlight 构建版本现在跟随 Release Tag，版本信息更统一。
- 修正 MCP 启动器路径配置说明，减少 AI 客户端接入时的路径配置错误。

完整记录请查看 [CHANGELOG.md](./CHANGELOG.md) 和 [v1.4.11 Release](https://github.com/cropflre/nowen-note/releases/tag/v1.4.11)。
...
> v1.4.11 重点改善笔记独立副本、搜索正确性、编辑器菜单、移动端 Markdown 预览、协作图片预览、共享附件、AI 布局和备份保留。升级后建议重点检查登录、笔记副本、搜索、编辑器、移动端 Markdown 预览、共享附件、图片预览和备份。镜像回滚不等于数据库回滚，生产环境必须保留独立备份。
