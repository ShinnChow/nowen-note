...
v1.4.14 聚焦 **移动端图片与媒体体验、大备份恢复稳定性、笔记切换安全、导出兼容性与移动端细节优化**，继续提升 Nowen Note 在手机、NAS 和大数据量场景下的可靠性。

- **移动端图片支持复制 / 剪切 / 粘贴**：富文本图片节点可以在同一篇笔记内安全移动或复制，保留尺寸、旋转、翻转等属性；剪切未选择目标位置时不会误删原图。
- **视频附件恢复更稳定**：视频播放源会跟随附件签名地址刷新，签名晚到或刷新后可自动恢复播放器，减少 Android / WebView 下出现“视频加载失败”的情况。
- **大备份恢复继续强化**：大型完整备份支持流式恢复，并补充启动守卫、生产入口校验和 TypeScript 类型修复，降低大归档恢复时的资源与失败风险。
- **笔记切换更安全**：修复旧请求、旧保存确认和激活竞态导致上一条笔记重新覆盖当前笔记的问题，切换过程更稳定。
- **导出与渲染修复**：团队空间根目录文档可正确进入单篇 Markdown + 附件 ZIP 导出；Mermaid 图表在图片导出中会正确渲染并栅格化。
- **移动端细节优化**：文件详情辅助信息默认折叠，右键菜单保持在视口内，长按不再误选笔记列表文字，并修复权限分享与公共空间导航问题。

查看：[v1.4.14 Release](https://github.com/cropflre/nowen-note/releases/tag/v1.4.14) · [完整更新日志](./CHANGELOG.md)
...
- 富文本图片节点新增移动端复制、剪切和粘贴能力，在同一篇笔记内可以重新排版图片，不需要重新上传附件。
- 图片移动会保留尺寸、旋转、翻转等节点属性；剪切后未选择目标位置时保持原图，避免误删。
- 视频节点会跟随附件签名地址变化重新解析播放源，签名刷新后可自动重建播放器并恢复加载。
- 图片操作面板滚动时继续锚定当前图片，旋转后仍保持节点选中，减少移动端连续编辑时的操作中断。
- 文件详情中的外链分享和文件元数据在移动端默认折叠，预览区域优先展示，减少长页面占用。

### 稳定性、备份与笔记切换

- 大型完整备份支持流式恢复，补充启动守卫、生产入口校验与相关回归测试，降低大归档恢复时的内存和失败风险。
- 修复流式恢复中的 TypeScript `this` 类型问题，确保 Docker 后端构建能够通过严格类型检查。
- 修复笔记快速切换时旧激活请求、旧保存确认和提交竞态覆盖当前笔记的问题，避免上一条笔记重新“跳回来”。
- 浏览器剪藏扩展补充登录会话自动续期，减少长时间使用后会话过期导致的剪藏中断。

### 导出、Markdown 与移动端细节

- 团队空间根目录文档现在能够正确进入 Markdown + 附件 ZIP 的单篇导出范围。
- Mermaid 图表在 PNG / JPG 等图片导出前会正确渲染并栅格化，避免导出结果缺图。
- Markdown 选中文字支持语法符号自动包裹，减少手动输入成对标记的操作成本。
- 修复移动端权限分享与公共空间导航、长按误选文本、短视口右键菜单越界等问题。
- 展开后的移动端工具栏支持完整滚动，并在输入法弹出时保持顶部工具栏布局稳定。

完整记录请查看 [CHANGELOG.md](./CHANGELOG.md) 和 [v1.4.14 Release](https://github.com/cropflre/nowen-note/releases/tag/v1.4.14)。
...
> v1.4.14 重点改善移动端图片与媒体操作、大型完整备份恢复、笔记切换稳定性和导出兼容性。升级后建议重点检查移动端图片复制 / 剪切 / 粘贴、视频播放、完整备份恢复、快速切换笔记以及 Markdown + 附件 ZIP / Mermaid 图片导出。镜像回滚不等于数据库回滚，生产环境必须保留独立备份。
