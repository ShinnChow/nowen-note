...
| | |
| --- | --- |
| **数据真正属于你** | 支持 Docker / NAS 自托管，数据库、附件和备份均由你管理，可接入 S3、Cloudflare R2 与 MinIO。 |
| **一棵树管理全部内容** | 文件夹、富文本和 Markdown 文档统一组织，支持拖拽、排序、筛选、权限继承和共享内容展示。 |
| **从短笔记到大型文档** | 富文本与 Markdown 双编辑器覆盖日常记录、技术文档和长文写作，并针对大文档提供渐进加载、增量同步与安全降级。 |
| **个人使用与团队协作兼顾** | 从标签、任务、AI 到工作区、权限、实时协作和公开知识空间，无需在多套工具之间来回切换。 |

## 核心能力

| 模块 | 当前能力 |
| --- | --- |
| **统一知识树** | 文件夹、富文本与 Markdown 文档混合组织；无限层级、拖拽排序、快速创建、筛选搜索、回收站、节点权限与共享目录展示；移动端支持最近优先、逐层浏览与树形模式。 |
| **富文本与 Markdown** | Tiptap 3 富文本、CodeMirror 6 Markdown、实时预览与分屏、大纲、格式刷、斜杠命令、表格、代码块、KaTeX、Mermaid、脚注、媒体嵌入、评论与版本历史。 |
...
| **协作、权限与分享** | Yjs + WebSocket 实时协作、工作区与成员角色、目录级权限、集中分享管理、分享密码与有效期、访客评论、公开知识空间。 |
| **导入、导出与迁移** | 支持 Markdown、Word / DOCX、网页 URL、微信公众号文章、SingleFile HTML、思源等导入路径；支持 Markdown、PDF、Word、图片和完整 ZIP 导出，并提供权限映射、冲突预检、报告与受控撤销。 |
| **附件与存储** | 本地附件按 `YYYY/MM` 归档，支持缩略图、引用检查、孤儿清理和历史网络图片本地化；可使用本地磁盘或 S3 / R2 / MinIO 对象存储。 |
| **账号与安全** | 多账号登录历史、记住账号 / 自动登录、远程服务连接、会话撤销、2FA、Personal API Token 权限范围、审计日志与安全化附件访问。 |
| **自动化与开放能力** | 自动备份、邮件备份、Webhook、插件系统、OpenAPI、TypeScript SDK、CLI、MCP Server 与浏览器剪藏扩展。 |
| **多端访问** | Web、Electron（Windows / macOS / Linux）、Android、iOS 工程、HarmonyOS 工程，以及 Docker / NAS 部署。 |
...
| `JWT_SECRET` | 自动生成并持久化 | 登录与会话签名；多实例部署时必须统一配置 |
...
| **Android** | Releases APK 或在 `frontend/` 下使用 Capacitor 构建 | 正式维护；支持系统分享导入、沉浸式编辑和移动端知识树 |
...
| **存储** | SQLite、本地附件、S3 / Cloudflare R2 / MinIO；PostgreSQL 处于适配验证阶段 |
...
README 只维护稳定的项目定位、能力范围和部署方式，不再内嵌大量自动生成的提交记录。
