...
function verifyRemoteRelease({ repo, tag, version }) {
  ...
  try {
    ...
    // macOS currently uses the browser/manual download policy. CI still publishes both
    // x64 and arm64 ZIP companions, while latest-mac.yml is generated by the final
    // architecture build and may reference only one of them. Do not reject the other
    // same-version Mac ZIP as an orphan updater binary; both DMG/ZIP architecture pairs
    // are checked separately by the desktop release completeness gate above.
    const unreferencedDesktopAssets = assets
      .map((asset) => asset.name)
      .filter((name) => isDesktopUpdaterAsset(name) && !referenced.has(name) && !isMacManualDownloadZip(name));
